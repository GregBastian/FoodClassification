...
def getTrainingData(training_wd):
    ...
    with open('TRAINING DATABASE.csv') as csv_file:
        ...
        for row in csv_reader:
            ...
            food_name = row[1]
            lbp = ast.literal_eval(row[2])
            cm = ast.literal_eval(row[3])
            preloadedData = [food_name,filename,lbp,cm]
...
#        if testItem.file_name == 'testing_Oreo_12.jpeg':
#            print(testItem.file_name, '<- Filename\n')
#            print(closestNeighbors,'<- Closest Neighbours\n')
#            print(result, '<- Result')
#            print('===============================')
        predicted_y.append(result)
    finalResultsF1 = classification_report(true_y, predicted_y)
    finalResultsAcc = accuracy_score(true_y, predicted_y)
    print(finalResultsF1)
    print('Accuracy ->',round(finalResultsAcc,3))
